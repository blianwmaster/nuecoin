...
void MiningPage::startPoolMining()
{
    ...
    if(port.length() == 0){
        reportToList(tr("please input the server port"),ERROR, NULL);
        return;
    }
    if(ui->usernameLine->text().length() == 0){
        reportToList(tr("Username should not be empty"), ERROR, NULL);
        return;
    }
    if(password.length() == 0){
        reportToList(tr("Password should not be empty"), ERROR, NULL);
        return;
    }
    ...
    args << "-s" << ui->scantimeBox->text().toLatin1();
    args << "-o" << urlLine.toLatin1();
    args << "-O" << QString("%1:%2").arg(ui->usernameLine->text(), ui->passwordLine->text()).toLatin1();
    args << "-t" << ui->threadsBox->text().toLatin1(); // --threads=N number of miner threads <default:number of processors>
    args << "-D"; // --debug enable debug output
    ...
    ui->shareCount->setText(tr("Accepted: 0 - Rejected: 0"));
...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList(tr("Miner failed to start. Make sure you have the minerd executable and libraries in the same directory as NUE-Qt."), ERROR, NULL);
    }
}
...
void MiningPage::updateSpeed()
{
    ...
    if (totalThreads == initThreads)
        ui->mineSpeedLabel->setText(QString(tr("Speed: %1 khash/sec - %2 thread(s)")).arg(speedString, threadsString));
    else
        ui->mineSpeedLabel->setText(QString(tr("Speed: ~%1 khash/sec - %2 thread(s)")).arg(speedString, threadsString));

    ui->shareCount->setText(QString(tr("Accepted: %1 (%3) - Rejected: %2 (%4)")).arg(acceptedString, rejectedString, roundAcceptedString, roundRejectedString));
