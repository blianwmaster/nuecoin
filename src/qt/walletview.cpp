    ...
    walletModel(0)
{
    ...
    sendCoinsByDelegatePage = new SendCoinsByDelegateDialog(gui);
    ...
    // Clicking on "Send Coins" in the address book sends you to the send coins by delegate tab
    connect(addressBookPage, SIGNAL(sendCoins(QString)), this, SLOT(gotoSendCoinsByDelegatePage(QString)));
...
{
    gui->getSendCoinsByDelegateAction()->setChecked(true);
...
{
    // URI has to be valid
    if (sendCoinsByDelegatePage->handleURI(strURI))
    {
        gotoSendCoinsByDelegatePage();
