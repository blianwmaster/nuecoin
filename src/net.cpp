...
CAddress GetLocalAddress(const CNetAddr *paddrPeer)
{
    ...
    if (GetLocal(addr, paddrPeer))
    {
        ...
        if (!ret.IsTor()) {
            ...
            double const fraction = atof(mapArgs["-advertisedbalance"].c_str());
            ret.advertised_balance = fraction * pwalletMain->GetBalance();
        }
    }
    return ret;
}
...
CNode* FindNode(const CService& addr, bool verified)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
...
void ThreadSocketHandler()
{
    ...
    loop
    {
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            bool is_tor = false;
#ifdef USE_IPV6
            struct sockaddr_storage sockaddr_listen;
            struct sockaddr_storage sockaddr;
#else
            struct sockaddr sockaddr_listen;
            struct sockaddr sockaddr;
#endif
            socklen_t len_listen = sizeof(sockaddr_listen);
            ...
            else
            {
                if (
                    ...
                    CAddress addr_listen;
                    if (
                        addr_listen.SetSockAddr(
                            (const struct sockaddr*)&sockaddr_listen
                        )
                    ) {
                        is_tor = addr_listen.IsLocal();
                    }
                }
                printf("accepted connection %s\n", addr.ToString().c_str());
                CNode* pnode = new CNode(hSocket, addr, "", true, is_tor);
                pnode->AddRef();
                {
                    LOCK(cs_vNodes);
                    vNodes.push_back(pnode);
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "NUE " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"web01","115.28.20.243"},
    {"web02","123.127.207.152"},
    {"web03","61.147.121.229"},
    {"web04","183.133.105.13"},
    {"web05","123.171.142.140"},
    {"web06","39.188.250.192"},
    {"web07","115.215.7.147"},
    {"web08","192.126.119.206"},
    {"web09","192.126.119.204"},
    {"web10","192.126.123.116"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. NUE is probably already running."), addrBind.ToString().c_str());
        else
...
std::map<CAddress, uint64> ListAdvertisedBalances()
{
    std::map<CAddress, uint64> result;
...
void InitializeDelegateBind(
    ...
    PushOffChain(sufficient, "to-sender", rawTx);
}
