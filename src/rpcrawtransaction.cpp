...
// Copyright (c) 2014 The NUE developers
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid NUE address: ")+input.get_str());
...
Value createtransferexpiry(const Array& params, bool fHelp)
{
    ...
    if (!destination_address_parsed.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NUE address");
...
Value createtransferfinalize(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "createtransferfinalize <bind tx> <destination address>\n"
            "Finalize transfer from sender bind transaction\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction is not stored in the wallet\n"
            "or transmitted to the network.");
    ...
    CScript destination;
    destination.SetDestination(destination_address_parsed.Get());

    CTransaction const rawTx = CreateTransferFinalize(
        pwalletMain,
        bind_tx,
        destination
    );
...
Value createtransferescrow(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "createtransferescrow <bind tx> <delegate tx> <tor address> <bind nonce> <transfer nonce> <destination address>\n"
...
Value createtransfercommit(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 5)
        throw runtime_error(
            "createtransfercommit <bind tx> <tor address> <bind nonce> <transfer nonce> <destination address>\n"
            "Commit transfer from delegate bind transaction\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction is not stored in the wallet\n"
            "or transmitted to the network.");
    ...
    uint256 const bind_tx = ParseHashV(params[0], "parameter 1");
    string const tor_address = params[1].get_str();
    boost::uint64_t const bind_nonce = params[2].get_uint64();
    boost::uint64_t const transfer_nonce = params[3].get_uint64();
    string const destination_address = params[4].get_str();
    ...
    destination_script.SetDestination(destination_address_parsed.Get());

    CTransaction const rawTx = CreateTransferCommit(
        pwalletMain,
        bind_tx,
        tor_address_parsed,
        bind_nonce,
...
Value createdelegatebind(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 5)
        throw runtime_error(
            "createdelegatebind <tor address> <nonce> <transfer amount> <expiry> <recovery coin address>\n"
            "Create a delegate bind transaction\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.");

    RPCTypeCheck(params, list_of(str_type)(int_type)(real_type)(int_type)(str_type));

    string const tor_address = params[0].get_str();
    boost::uint64_t const nonce = params[1].get_uint64();
    uint64_t const transferred = AmountFromValue(params[2]);
    boost::uint64_t const expiry = params[3].get_uint64();
    string const recover_address = params[4].get_str();

    CBitcoinAddress const recover_address_parsed(recover_address);
    if (!recover_address_parsed.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NUE address");
...
Value createsenderbind(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "createsenderbind <tor address> <nonce> <transfer amount> <fee amount> <expiry> <recovery coin address>\n"
            "Create a sender bind transaction\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.");

    RPCTypeCheck(params, list_of(str_type)(int_type)(real_type)(real_type)(int_type)(str_type));

    string const tor_address = params[0].get_str();
    boost::uint64_t const nonce = params[1].get_uint64();
    uint64_t const transferred = AmountFromValue(params[2]);
    uint64_t const fee = AmountFromValue(params[3]);
    boost::uint64_t const expiry = params[4].get_uint64();
    string const recover_address = params[5].get_str();
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid NUE address: ")+s.name_);
...
Value fundaddressbind(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "fundaddressbind <hex string>\n"
            "Fund signed raw address bind transaction using coins in the wallet\n"
            "Returns a raw transaction with funding input(s) (hex-encoded string)"
    ...
    CTransaction unfundedTx(txVariants[0]);

    CTransaction fundedTx = FundAddressBind(pwalletMain, unfundedTx);
...
Value signsenderbind(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signsenderbind <hex string> <signing sender>\n"
            "Sign identity for raw sender bind transaction (serialized, hex-encoded).\n"
            "Second argument is a standard coin address to sign the identity\n"
            "Returns a raw transaction with signature(s) (hex-encoded string)"
            + HelpRequiringPassphrase());

    RPCTypeCheck(params, list_of(str_type)(str_type), true);
    ...
    SignSenderBind(pwalletMain, mergedTx, address);
...
Value signdelegatebind(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signdelegatebind <hex string> <signing delegate>\n"
            "Sign identity for raw delegate bind transaction (serialized, hex-encoded).\n"
            "Second argument is a standard coin address to sign the identity\n"
            "Returns a raw transaction with signature(s) (hex-encoded string)"
            + HelpRequiringPassphrase());
