...
uint256 hashGenesisBlock("0x9326c4e757b6efda7e742857025c90954fbb20f08023447857bb627ce050a454");
...
const string strMessageMagic = "NUE Signed Message:\n";
...
{
    // NUE: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nOriginalInterval = nTargetTimespan / nTargetSpacing;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NUE: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NUE: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // NUE: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9326c4e757b6efda7e742857025c90954fbb20f08023447857bb627ce050a454");
    }
...
bool InitBlockIndex() {
	...
	// TODO: add correct genesis block for bcrypt-based PoW scheme
        const char* pszTimestamp = "the New Universal Euro Release 2014/7/15 20:53:39";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405428819;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 120612199;
        ...
        assert(block.hashMerkleRoot == uint256("0x5be53c7758eab4b1e1c735e849602e00ee7682f004cd8f29007bba4429787956"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // NUE: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NUEMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NUEMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NUEMiner(CWallet *pwallet)
{
    printf("NUEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("NUE-miner");
    ...
    try { loop {
        ...
        printf("Running NUEMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            loop
            {
                std::string const hash_data = bcrypt_iterated(
                    std::string(
                        reinterpret_cast<char const*>(BEGIN(pblock->nVersion)),
                        80
                    )
                );
                memcpy(
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nGenerateThreads; i++)
        minerThreads->create_thread(boost::bind(&NUEMiner, pwallet));
}
