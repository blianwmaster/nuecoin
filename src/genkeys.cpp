...
// fork from heavycoin branch v1.0 , genkeys.cpp 
#include <getopt.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
static int PRIVATE_KEY_NUE_BYTE = 128;
static int PUBLIC_KEY_NUE_BYTE= 52;   // NUE addresses start with N
...
// WIF = PRIVATE_KEY_NUE_BYTE + Hash(secret) + 4 byte checksum (take first 4 bytes from Hash(PRIVATE_KEY_NUE_BYTE + Hash(secret)) )
std::vector<unsigned char> getPrivateKeyWIF(std::vector<unsigned char> secret)
{
    uint256 digest = Hash(secret.begin(), secret.end());

    std::vector<unsigned char> vch(1, PRIVATE_KEY_NUE_BYTE);
...
// Get address
// Address = PUBLIC_KEY_NUE_BYTE + Hash160(ECDSAPubKey) + 4 byte checksum
// ECDSAPubKey = EC_KEY_generate_pubkey(Hash(secret))
// checksum = first 4 bytes from Hash(PUBLIC_KEY_NUE_BYTE + Hash160(ECDSAPubKey))
std::vector<unsigned char> getAddress(const std::vector<unsigned char> &vchSecret)
{
    ...
    std::vector<unsigned char> vch(1, PUBLIC_KEY_NUE_BYTE);
...
void printOptions(int argc, char **argv)
{
	...
	std::cout << "\t--dec64 [string]\t\tdecode base64 string"<< std::endl;
	std::cout << "\t--phrase [string]\t\tset secret (brainwallet)"<< std::endl;
        std::cout << "\t--testnet       \t\tgenerate keypair and address for testnet"<< std::endl;
	std::cout << std::endl;
}
       ...
       while (1)
         {
           ...
           switch (c)
             ...
             case 'p':
               ...
               break;
			 case 't':
			   PUBLIC_KEY_NUE_BYTE = 112;
			   PRIVATE_KEY_NUE_BYTE = 239;
...
int main(int argc, char **argv)
{
    ...
    std::string name("NUE ");	
    if ( PRIVATE_KEY_NUE_BYTE == 239 ) {
	name += "[testnet] ";
	}

    std::cout << std::endl;
    std::cout << name << "Private Key:\t" << EncodeBase58(PrivateKeyWIF) << std::endl;
    std::cout << name << "Public Key:\t" << ToHex(pubKeybn, pubKeybn + ECDSA_PUBKEY_DIGEST_LENGTH) << std::endl;
    std::cout << name << "Address:\t\t" << EncodeBase58(Address) << std::endl;
    std::cout << std::endl;
