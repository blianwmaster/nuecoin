...
private:
    ...
    std::map<uint64, std::vector<unsigned char> > join_nonce_delegates;

    std::map<std::vector<unsigned char>, uint64> delegate_nonces;

    std::map<std::vector<unsigned char>, uint256> sender_binds;

    std::map<uint160, std::vector<unsigned char> > hash_delegates;

    std::set<std::pair<CNetAddr, uint64> > address_binds;
...
public:
    ...
    void store_address_bind(CNetAddr const& address, uint64 const& nonce);

    std::set<std::pair<CNetAddr, uint64> >& get_address_binds();
